...
from gym_pybullet_drones.sb3_envs.BaseAviary import BaseAviary
...
class BaseRLAviary(BaseAviary):
    ...
    def _observationSpace(self):
        ...
        obs_lower_bound = np.full((self.GLOBAL_MAX_NUM_DRONES, obs_len), lo, dtype=np.float32)
        obs_upper_bound = np.full((self.GLOBAL_MAX_NUM_DRONES, obs_len), hi, dtype=np.float32)
    ...
    def _computeObs(self):
        ...
        obs = np.zeros((self.GLOBAL_MAX_NUM_DRONES, obs_len_per_drone), dtype=np.float32)
        ...
        for i in range(N):
            ...
            while len(rel_neighbors) < self.GLOBAL_MAX_NUM_DRONES:
