...
class BaseAviary(gym.Env, MathematicalFlock):
    ...
    def __init__(self,
        ...
        self.GLOBAL_MAX_NUM_DRONES = 12
        self.MAX_NUM_DRONES = max_num_drones   
        self.MIN_NUM_DRONES = min_num_drones
        ...
        self.INIT_XYZS = self.initialize_drone_positions(num_drones=self.NUM_DRONES)
        self._housekeeping()
    ...
    ################################################################################
    def initialize_drone_positions(self, num_drones=None, spawn_origin=[0,0], spacing=1.75):
        ...
        if num_drones <= 4:
            ...
            for i in range(num_drones):
                x = i * spacing
                positions.append([x + spawn_origin[0], y + spawn_origin[1], self.DRONE_TARGET_ALTITUDE])
        else:
            n_row1 = num_drones // 2
            n_row2 = num_drones - n_row1
            y_spacing = spacing
            # Row 1
    ...
    def reset(self, seed=None, options=None):
        ...
        print(f"new episode with number of dornes: {self.NUM_DRONES}")
        ...
        initial_obs = {i: self._computeObs(i) for i in range(self.NUM_DRONES)}
        initial_info = {"__all__": {}}
    ...
    def step(self, action):
        ...
        dict
            Rewards per drone.
        dict
            Terminated flags per drone.
        dict
            Truncated flags per drone.
        dict
            Info per drone.
        """
        self.step_counter_A += 1

        # ------------------ VIDEO / PNG FRAME CAPTURE ------------------ #
        if self.RECORD and not self.GUI and self.step_counter % self.CAPTURE_FREQ == 0:
            ...
            Image.fromarray(np.reshape(rgb, (h, w, 4)), 'RGBA').save(
                os.path.join(self.IMG_PATH, f"frame_{self.FRAME_NUM}.png")
            )
            self.FRAME_NUM += 1

            # Save onboard drone images if enabled
            if self.VISION_ATTR:
                for i in range(self.NUM_DRONES):
                    ...
                    self._exportImage(
                        ...
                        path=f"{self.ONBOARD_IMG_PATH}/drone_{i}/",
                        frame_num=int(self.step_counter / self.IMG_CAPTURE_FREQ)
                    )

        # ------------------ GUI / User Debug Input ------------------ #
        ...
        if self.USE_GUI_RPM:
            ...
            clipped_action = np.reshape(self._preprocessAction(action), (self.NUM_DRONES, self.ACTION_SPACE))
        ...
        # ------------------ COMPUTE RETURNS ------------------ #
        obs = {i: self._computeObs(i) for i in range(self.NUM_DRONES)}
        reward = {i: self._computeReward(i) for i in range(self.NUM_DRONES)}
        done = {i: self._computeTerminated(i) for i in range(self.NUM_DRONES)}
        done["__all__"] = all(done.values())
        truncated = {i: self._computeTruncated(i) for i in range(self.NUM_DRONES)}
    ...
    def _computeReward(self, drone_id):
        """Computes the current reward for the specifed drone.
