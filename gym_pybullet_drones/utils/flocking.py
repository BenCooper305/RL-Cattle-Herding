...
class MathematicalFlock():
    ...
    def update(self, *args, **kwargs):
        ...
        local_clustering = self._local_clustering(herd_states, shepherd_states, k=0.5) #Keeps herd members grouped locally
        global_clustering = self._global_clustering(herd_states, shepherd_states) #Keeps herd together on a larger scale
        flocking = self._flocking(herd_states, shepherd_states) #aligns herd velocoites (cohesion alignment rules)
        remain_in_bound_u = self._calc_remain_in_boundary_control(herd_states, self._boundary, k=5.0) #pushes agents back inside simulation limits

        #Calculate herd density
        herd_density = self._herd_density(herd_states=herd_states,shepherd_states=shepherd_states)
    ...
    def _herd_density(self, herd_states: np.ndarray,shepherd_states: np.ndarray):
        ...
        for idx in range(herd_states.shape[0]):
            # Density
            neighbor_idxs = alpha_adjacency_matrix[idx]
            density = self._calc_density(idx=idx, neighbors_idxs=neighbor_idxs,
                                         herd_states=herd_states)
